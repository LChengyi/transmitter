...
\endverbatim
*/
int stimulator::Open_serial()
{
  return this->Sport.serial_openport();
}
/*!
...
#ifdef _TTY_POSIX_
  norb=this->Sport.serial_read_timeout(Read_Buffer, this->sent_updates-this->successful_updates-this->unsuccessful_updates);
#else
			 ...
			 )
{
  ...
  *Channel_Stim=0;
  ...
  for(i=0;i<nc;i++) 	
    ...
    *Channel_Stim|=(unsigned int)pow(2.0, S_Channel_Stim[i]-1);
  //  printf("Test %f \n",pow(2.0,2.0));
  return 0;
}
			 ...
			 )
{
  ...
  *Channel_Lf=0;
  for(i=0;i<n_lf;i++)
    ...
    *Channel_Lf|=(unsigned int)pow(2.0,S_Channel_Lf[i]-1);
	return 0;  
}
